...
class PlayerInterface:
    def __init__(self):
        ...
        # Music names
        self.knownMusics = ['Royalty', 'Haunted House', 'Symphony', 'demons', 'I Hear Sleigh Bells']
        self.royalty = nsii.new_image('image/Royalty.ppm')
        self.hauntedHouse = nsii.new_image('image/Haunted House.ppm')
        self.symphony = nsii.new_image('image/Symphony.ppm')
        self.demons = nsii.new_image('image/Demons.ppm')
        self.iHearSleighBells = nsii.new_image('image/I Hear Sleigh Bells.ppm')

    def showMusic(self, musicId, list):
        """
        Show the name of the music inside the correct tab
        :param musicId: Identifier of the music
        :param list: List on which the function searches
        """
        musicName = list[musicId + self.pageId * 3]
        if list == player.queue:
            musicName = musicName.strip(player.musicPath)
        musicName = musicName.strip(".wav")

        if musicName in self.knownMusics:
            if musicName == 'Royalty':
                self.royalty.size = (resizeX(112), resizeY(16))
                self.royalty.pos = (resizeX(46), resizeY(14 + 18 * musicId))
                self.royalty.show(hide=(255, 255, 255))
            elif musicName == 'Haunted House':
                self.hauntedHouse.size = (resizeX(112), resizeY(16))
                self.hauntedHouse.pos = (resizeX(46), resizeY(14 + 18 * musicId))
                self.hauntedHouse.show(hide=(255, 255, 255))
            elif musicName == 'Symphony':
                self.symphony.size = (resizeX(112), resizeY(16))
                self.symphony.pos = (resizeX(46), resizeY(14 + 18 * musicId))
                self.symphony.show(hide=(255, 255, 255))
            elif musicName == 'demons':
                self.demons.size = (resizeX(112), resizeY(16))
                self.demons.pos = (resizeX(46), resizeY(14 + 18 * musicId))
                self.demons.show(hide=(255, 255, 255))
            elif musicName == 'I Hear Sleigh Bells':
                self.iHearSleighBells.size = (resizeX(112), resizeY(16))
                self.iHearSleighBells.pos = (resizeX(46), resizeY(14 + 18 * musicId))
                self.iHearSleighBells.show(hide=(255, 255, 255))
        else:
            pass
            #  nsii.print((resizeX(46), resizeY(20 + 18 * (musicId - 1))), musicName)

    def showInterface(self):
        """Manages the images that are shown on the screen"""
        # Music Tab
        if self.menuState == 0:
            if self.pageId < self.numPage:  # 3 musics are available in this page
                self.musicTab3.size = nsii.size
                self.musicTab3.show()
                self.showMusic(2, self.musicList)
                self.showMusic(1, self.musicList)
                self.showMusic(0, self.musicList)
        ...
        # Queue Tab
            ...
            if self.pageId < len(player.queue) // 3:  # 3 musics are available in this page
                ...
                self.queueTab3.show()
                self.showMusic(2, player.queue)
                self.showMusic(1, player.queue)
                self.showMusic(0, player.queue)
    ...
    def buttonManagement(self):
        """
        Manages all the buttons presses
        """
...
if __name__ == '__main__':
    ...
    while True:
        ...
        if player.currentMusic:
            currentMusic = player.currentMusic.strip(player.musicPath).strip('.wav')
        else:
            currentMusic = 'En attente'

        nsii.name = f"Wave Music - {currentMusic}"
